...
- Length of stay at index hospitalization >= 7 days
...
The long-term outcomes were also modelled using Cox proportional hazards regressions with 
an interaction term between the comorbidity score and EF, thereby enabling separate 
estimates for each EF group, and presented in a Forest plot. 
The models were adjusted for the same variables as above. 

The outcome all-cause mortality was further analysed in univariable 
Cox proportional hazards regressions with each individual non-cardiac 
comorbity summarizing to the comorbidity score separately in each EF group. 
...
```{r, child = "../src/outforestefabstract.Rmd"}

```

\newpage

```{r, child = "../src/uniforestmort.Rmd"}
